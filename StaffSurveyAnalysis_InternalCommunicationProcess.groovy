//Please rate how effective current internal communication processes (see 5 above) are at conveying the following information:
//jquarks4s_answers(5).csv
int count = 0

Map<String, ArrayList<String>> map = new HashMap<String, ArrayList<String>>() // q, <not_good, poor, fair, good, excellent>

new File("/Users//SPC/Downloads/_survey/jquarks4s_answers(5).csv").eachLine { line ->
	String[] arr =  line.split(";")
	if (count != 0){
		for (int i = 12; i <= 67; i = i + 8){
...
map.keySet().each { question ->
	...
	if (poor + not_good >= 5 ) cat3 << question + " ; " + (poor + not_good)
